...
CI_LOG_URL=""
if [ -n "${CI_BASE_URL:-}" ]; then
	CI_LOG_URL="${CI_BASE_URL}/tab/build/detail/${GO_PIPELINE_NAME}/${GO_PIPELINE_COUNTER}/${GO_STAGE_NAME}/${GO_STAGE_COUNTER}/${GO_JOB_NAME}"
fi
...
function check_released() {
	...
	status=$(curl -LISsL "https://insights.nyc3.digitaloceanspaces.com/yum-beta/x86_64/do-agent.${VERSION}.amd64.rpm" \
		| grep 'HTTP/1.1')
